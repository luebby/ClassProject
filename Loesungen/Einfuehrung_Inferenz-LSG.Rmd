---
title: "Einführung in die Inferenz: Lösungen"
...
Sie ist mindestens bi-modal -- evt. sogar multi-modal. Ein Gipfel ist bei ca. $12.000$, der kleinere, mit weniger Werten bei ca $8.000$. (Wochentage!)
...
```{r VerteilungStiproLSG}
set.seed(1896)
gf_histogram( ~ births, data = sample(Births2015, size = 50))
```

Die bi-modalität findet sich auch in der Stichprobe wieder.
...
Anders als die Verteilung der Geburten selber ist die Stichprobenverteilung des Mittelwertes annährend symmetrisch, normalverteilt.
...
# Simulation
set.seed(1896)
Stiprovtlg250 <- do(1000)* mean( ~ births, data = sample(Births2015, size = 250))

# Histogramm
gf_histogram( ~ mean, data = Stiprovtlg250) %>%
  gf_vline(xintercept = mean(~births, data = Births2015))

# Standardfehler
sd( ~ mean, data = Stiprovtlg250)
```

Bei einem größeren Stichprobenumfang sinkt die Streuung des Mittelwertes ($se_{250}=`r sd( ~ mean, data = Stiprovtlg250)`$ im Verglech zu $se_{50}=`r sd( ~ mean, data = Stiprovtlg)`$), *nicht* die der Beobachtungen Stichprobe:

```{r sd250}
set.seed(1896)
do(3)* sd( ~ births, data = sample(Births2015, size = 50))
do(3)* sd( ~ births, data = sample(Births2015, size = 250))
```
...
Ja, hier wird der Mittelwert der Population ($\bar{x}=`r mean( ~ births, data = Births2015)`$) vom $95\,\%$-Konfidenzitervall überdeckt.
...
Da es aufgrund von Variabliltät, auch wenn $H_0$ stimmt, zu Unterschieden kommt. p-Wert von co. beantworten die Frage: wie wahrscheinlich ist eine mindestens so große Abweichung wie die beobachtete, wenn die Nullhypothese stimmt. 
...
```{r Visu-p-WertLSG}
gf_histogram( ~ diffmean, data = Nullvtlg) %>%
  gf_vline(xintercept = diffmean(births ~ wday, data = Births.MonTue))
```

Die beobachtete Differenz, kommt in der simulierten Permutations-Verteilung der Differenzen, d.h. der Verteilung, wenn die vorliegen würde, wenn die Nullhypothese stimmen würde, nicht vor. Der vorliegende Wert ist unter diesem Modell daher nicht plausibel -- und dies nährt Zweifel an der Gültigkeit der Nullhypothese.

Mit einem p-Wert

```{r pvalue}
prop( ~ (abs(diffmean) >= abs(diffmean(births ~ wday, data = Births.MonTue))), data = Nullvtlg)
```

kleiner als $1/1000$ würde hier $H_0$ verworfen werden und man könnte von einer signifikanten Differenz der Mittelwerte der Geburten an Mon- bzw. Dienstagen sprechen.

Ein klassischer t-Test kommt zum selben Ergebnis:

```{r t-test}
t.test(births ~ wday, data = Births.MonTue)
```

Für die Effektgröße nach Cohens d würde gelten:

```{r}
library(lsr)
cohensD(births ~ wday, data = Births.MonTue)
```

Also ein großer Effekt.
