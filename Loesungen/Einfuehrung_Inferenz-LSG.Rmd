...
Anders als die Verteilung der Geburten selber ist die Stichprobenverteilung des Mittelwertes annähernd symmetrisch, normalverteilt.
...
Bei einem größeren Stichprobenumfang sinkt die Streuung des Mittelwertes ($se_{250}=`r sd( ~ mean, data = Stiprovtlg250)`$ im Verglech zu $se_{50}=`r sd( ~ mean, data = Stiprovtlg)`$), *nicht* die der Beobachtungen in der Stichprobe:
...
Da es aufgrund von Variabilität, auch wenn $H_0$ stimmt, zu Unterschieden kommt. p-Wert von co. beantworten die Frage: Wie wahrscheinlich ist eine mindestens so große Abweichung wie die beobachtete, wenn die Nullhypothese stimmt. 
...
Die beobachtete Differenz, kommt in der simulierten Permutations-Verteilung der Differenzen, d.h. der Verteilung, die vorliegen würde, wenn die Nullhypothese stimmen würde, nicht vor. Der vorliegende Wert ist unter diesem Modell daher nicht plausibel -- und dies nährt Zweifel an der Gültigkeit der Nullhypothese.
